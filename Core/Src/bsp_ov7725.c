	...
	for(i = 0; i < 240; i++){
		for(j = 0; j < 320; j++){
			...
			if (i>= 120 && k<100){
				...
				red = ((Camera_Data & 0xF800) >> 11);
				green = ((Camera_Data & 0x07E0) >> 5);
				blue = (Camera_Data & 0x001F);

				// grayscale
				//frame[k][l] = 0.2989*red + 0.5870*green + 0.1140*blue;

				frame[k][l] = rgb_to_hsv(red, green, blue);
				trigger = 1;
				l++;
			}
		}
